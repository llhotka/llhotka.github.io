...
offers a lot of useful functionality now, hopefully with more to come
later through software upgrades. One feature that I'd certainly love
to try is V2H (vehicle-to-home) or V2G (vehicle-to-grid) as per [ISO
...
latter is one of the extra functions implemented in the MyBox software, I
haven't tried it though. A natural choice for Home Assistant users is
to use the [Mosquitto
broker](https://github.com/home-assistant/addons/blob/master/mosquitto/DOCS.md)
add-on. Its installation and configuration is very easy, one just has
to create a special Home Assistant account & password for use with the
broker.
...
portal](https://cloud.mybox.pro) the configuration of the wallbox can
...
maximum charge current immediately poped up as seen in @fig-listen.
...
single-phase operation a thus go with the charging power down to
...
appreciate more extensive technical documentation. 
